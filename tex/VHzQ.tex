...
\citet[e.g][]{Secrest2015, Assef2015, Stern2015, Assef2018, LaMassa2017, Glikman2018, LaMassa2019} 
...
we select objects with at least 8
measurements observed with durations over 30 days, in at least one
filter and with the average calibrated flux is greater than 0 and at
least $8\times$ the error on the average calibrated flux. 
%At least 8 original observations across at least 30 days in either WSA or VSA per 
% band, calibrated flux greater than zero over the whole time, and the
% error on the average flux is 8 times lower than this. 
A requirement of SNR$>3$ per `average' epoch, where `average epoch' is
then dependent on the averaging timescales (which as we see can vary).
The clipped median and standard deviation is then calculated 
\begin{equation}
     1.48 \times \sigma_{median} / \bar{\epsilon}
\end{equation}
where $\bar{\epsilon}$ is the mean of the error in each point in the
...
with an ``average'' light curve sampling over e.g. a month, or two weeks. 
%%
