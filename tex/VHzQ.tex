...
%We note there are sources in the OB epochs with negative flux and they tend to outnumber the ones with positive flux. This is apparent for the average $H$  and $K_{s}$ flux for CFHQS J0216-0455 at MJD=56900, which is slightly negative, and so a default magnitude is reported. Interestingly it does this in both $H$ and $K_{s}$ filters, so perhaps something real is going on, but again, it is a QSO not much brighter than the detection limit, so one should be very careful with over interpretation. 
