...
\maketitle
\begin{abstract}
We assemble a catalogue of {\bf 488} spectroscopically confirmed very high
...
assembled list of the {\bf 488} $z\geq5.00$ VH$z$Qs that we have
...
observe in the $Z$-band.
  ...
  the positions of our {\bf 488} VH$z$Qs. This database is world-readable and
...
\section{Results}
Having collated the sample of {\bf 488} VH$z$Qs, and obtained their near-
...
deg$^2$). Crossmatching our catalogue of {\bf 488} VH$z$Qs with these
...
{\bf  \item Krisztina Perger for key discussions about missing objects after the pre-print appeared. }
...
{\bf In the journal submitted version of this paper, as well as the
arXiv v1, we had 463 VH$z$Qs selected from the literature which we
thought was the complete sample.  However, there were 25 (twenty-five)
objects which should have been in our sample but were not (priv. comm
from Jinyi Yang and Krisztina Perger).

We downloaded Tables 2, 3 \& 4 from \cite{YangJ2018a} and found all
matches with $2\arcsec$ and added the 16 objects that had $z\geq5$;
one object from Table 3 (SUHAW J1016+2541) and 15 from Table 4, see
Table~\ref{tab:new_qsos}.

We initially downloaded the 20190514 version of the Perger
catalogue\footnote{http://astro.elte.hu/~perger/catalog.html} again
matching on a $2\arcsec$ radius.  We found 10 objects (see
Table~\ref{tab:disagree_objs}) that were in both catalogues but had
disagreement on the coordinates.  Two particular odd examples were
SDSSJ0835+3227 and VA-13492.  In both cases the coordinates looked
similar to SDSSJ0835+3217 and SDSSJ1040-1155, respectively, and the
redshifts were identical to these quasars.

In the case of SDSSJ0835+3227, we checked the SDSS $z$-band
images\footnote{https://dr15.sdss.org}, and found a bright source at
the position of SDSSJ0835+3217 and no source at the position of
SDSSJ0835+3227, see Figure~\ref{fig:check_obj}. In the case of
VA-13492, the $i$-band magnitude was equivalent to SDSS J1040-1155
($i=25.9$ mag), so we have decided that both of these objects have
been transcribed incorrectly sometime in the past.

The objects from the submitted version of Ross \& Cross that have
changed position are flagged in Table~\ref{tab:disagree_objs}. The
Perger catalogue has been updated to the 20190724 version (which
matches our catalogue). A final test of matching to with $1\arcsec$
found another difference: we had downloaded the OGLEJ015531-75
coordinates from the published paper and Perger had done so from the
submitted arXiv paper - and \cite{Kozlowski2019} made a minor update
to the positional values in between.

We exclude DES0141-54 that is in the Perger catalogue from our final
sample. This object came from a conference proceedings
\cite{Belladitta2018}, which did not have arcsecond precision in its
coordinates so we could not perform our matched-aperture photometry.

Our final catalogue has 488 $z\geq$ QSOs, see Table~\ref{tab:new_qsos}.} 
